...
Logika
is_admin()
true kalau profiles.role = 'admin' untuk user yang login
is_active_member()
true kalau ada row members milik user dengan registration_status = 'active'
can_manage_announcements()
true kalau admin, atau member aktif dengan jabatan kode KETUA/WAKIL_KETUA
can_create_report()
true kalau member aktif dengan jabatan nama Bendahara, Sekretaris, Pokja I/II/III/IV
chat_is_non_admin(user_id)
true kalau role user bukan admin (chat sengaja dikunci hanya untuk non-admin)
is_post_active(post_id)
true kalau posts.expires_at > now()
...
Tidak ada anonymous access — semua fitur wajib user login duluan.
Self-registration anggota tidak langsung — insert ke members/profiles hanya lewat admin. Perlu klarifikasi ke pemilik project: apakah ada trigger/RPC untuk auto-provision saat signup, atau alurnya benar-benar manual approval oleh admin.
Role "Bendahara/Sekretaris/Pokja I-IV" adalah role khusus by positions.name untuk hak buat laporan — pastikan data positions sudah pakai nama persis itu (case-sensitive) karena dicek via p.name in (...).
Chat eksklusif non-admin — jangan tampilkan menu chat untuk user dengan profiles.role = 'admin'.
Beberapa tabel butuh RPC/Edge Function tambahan yang belum terlihat dari policy saja: mulai percakapan chat baru, insert report_recipients saat report dibuat, provisioning profiles/notification_inbox. Sebaiknya cek dulu ke pemilik backend apakah RPC ini sudah ada sebelum FE mulai integrasi fitur-fitur tsb.
Admin punya akses SELECT yang perlu dicek ulang di announcements (saat ini kondisinya malah mengecualikan admin).
