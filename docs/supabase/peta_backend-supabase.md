...
Tabel: profiles
Terhubung 1:1 ke auth.users (id sama dengan user auth). Kolom: name, role (admin / user), created_at, updated_at.
...
4. Laporan Kegiatan (Activity Report — PDF F4)
Tabel: reports
Form laporan kegiatan satu halaman: creator_name, creator_position, chairperson_name, activity_basis (≤150 char), activity_date, activity_time, activity_place (≤100), activity_name (≤150), participants (≤250), activity_description (≤800).
status saat ini terkunci ke 'sent' (constraint check).
Frontend: form terstruktur dengan character-count/limit sesuai constraint di atas (validasi FE harus sama persis dengan constraint DB agar tidak gagal insert). Setelah submit, panggil Edge Function generate-report-pdf (lihat bagian 6b) untuk generate PDF ukuran F4 sesuai template resmi TP PKK Kelurahan Warakas.
Tabel: report_media
Maks. 2 gambar dokumentasi per laporan (media_order 1=kiri, 2=kanan — constraint 1–2).
Kolom crop (crop_x/crop_y/crop_width/crop_height) disimpan agar preview crop dari FE bisa direproduksi ulang saat generate PDF. Satuan: fraksi 0–1 relatif terhadap dimensi gambar asli (bukan pixel) — komponen crop di FE wajib mengonversi ke fraksi sebelum insert ke kolom ini, karena Edge Function generate-report-pdf mengasumsikan format ini.
Frontend: komponen crop gambar sebelum upload, simpan koordinat crop (fraksi 0–1) persis ke kolom ini supaya hasil PDF konsisten dengan preview.
...
generate-report-pdf (status ACTIVE, verify_jwt: true) — generate PDF laporan kegiatan sesuai template resmi TP PKK Kelurahan Warakas, ukuran kertas F4. Dipanggil frontend saat tombol "Kirim Laporan" ditekan, dengan body { report_id }.
Alur internal:
Baca data reports + report_media memakai JWT user pemanggil (bukan service role) — RLS tetap berlaku, jadi hanya pembuat laporan / penerima (report_recipients) / admin yang bisa berhasil generate PDF laporan tersebut.
Render PDF F4 memakai library pdf-lib: logo PKK, field kegiatan (dasar kegiatan, hari & tanggal, pukul, tempat, acara, peserta), uraian kegiatan, 2 foto dokumentasi (di-crop sesuai crop_x/crop_y/crop_width/crop_height — fraksi 0–1), blok tanda tangan kosong (Mengetahui / Pembuat Laporan), serta footer watermark logo transparan + teks brand "My_PKK-Warakas" + pernyataan keaslian dokumen untuk arsip.
Foto dokumentasi diambil dari bucket report-media (public) via storage_path di report_media, menggunakan service-role client (baca saja).
Hasil PDF di-upload ke bucket privat report-pdfs dengan nama file {report_id}.pdf (upsert), menggunakan service-role client.
Response berisi signed URL (berlaku 1 jam) untuk keperluan download / share WhatsApp di FE.
Frontend: panggil lewat supabase.functions.invoke('generate-report-pdf', { body: { report_id } }) — bukan fetch() manual — supaya header Authorization (JWT user aktif) otomatis terpasang dan RLS tetap berlaku sesuai desain di atas.
...
7. Storage Buckets
report-media (public) — foto dokumentasi laporan, path per report_media.storage_path.
report-pdfs (privat) — hasil PDF laporan, nama file {report_id}.pdf. Policy SELECT: pembuat laporan (reports.created_by), penerima (report_recipients), atau admin. Tidak ada policy INSERT untuk role authenticated — penulisan hanya lewat service role di Edge Function generate-report-pdf.
...
Kompleks (form + crop + PDF) — backend sudah siap, termasuk Edge Function generate PDF
Chat
...
Semua tabel RLS aktif — sebelum mulai coding tiap modul, sebaiknya cek dulu isi policy RLS masing-masing tabel agar tahu persis siapa boleh insert/select/update apa.
